...
use app\ims\controller\PricingController;
...
//        halt($roomList);
        ...
        foreach($roomList as $k=>$v){
            ...
            $roomInfo = $this->formateData($roomModel->field('room_amount')->where('room_id',$v['room_id'])->find());
...
/*        if(!empty($seasonList)){
            ...
            $strSeasonList = $seasonList['season_unqid'];

            $conRoomModel = new ContractRoomModel();

            $roomList = $this->formateData($conRoomModel->field('id as con_room_id,contract_id,season_unqid,package_unqid,room_id as room_id2,pricing_mode,total_price,adult_fare,child_fare,extra_child_fare,room_price')->where("room_id in ($strRoomList) AND season_unqid in ('$strSeasonList')")->select());

            if(empty($newRoomList) || empty($roomList)){
                return getErr('没有房型信息');
            }

            foreach($newRoomList as $k=>$v){
                foreach($roomList as $m=>$n){
                    if($v['room_id'] == $n['room_id2']){
                        if(!is_array($n['adult_fare'])){
                            $n['adult_fare'] = json_decode($n['adult_fare'],true);
                        }

                        if(!is_array($n['extra_child_fare'])){
                            $n['extra_child_fare'] = json_decode($n['extra_child_fare'],true);
                        }

                        if(!is_array($n['room_price'])){
                            $n['room_price'] = json_decode($n['room_price'],true);
                        }

                        if(empty($v['room_amount'])){
                            $v['room_amount'] = 0;
                        }

                        $returnArr[] = array_merge($v,$n);
                    }
                }
            }
            ...
            foreach($newRoomList as $k=>$v){
                ...
                $returnArr[] = array_merge($v,$nullArr);
            }
        }

        return getSucc($returnArr);
*/
