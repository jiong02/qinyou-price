...
foreach($str1 as $k=>$v){
    $section->addText($v,array(),array('spacing'=>10));
}


$str11 = "乙方:遇岛（北京）国际旅行社有限公司
...
";
	    ...
	    foreach($str11 as $k=>$v){
		    $section->addText($v,array(),array('spacing'=>95,'spaceBefore'=>20));
	    }
...
foreach($wordData['hotel_data']['key'] as $k=>$v){
	if($v == '酒店或度假村名称'){
		$cell = $table->addCell(90000,$cellStyle);
		$cell->addText($v);
	}else if($v == '房型(中英文房型)'){
		$cell = $table->addCell(90000,$cellStyle);
		$cell->addText($v);
...
foreach($wordData['hotel_data']['value'] as $k=>$v){
    ...
    $table->addRow();
    for($i=0;$i<7;$i++){
        $cell = $table->addCell(9000,$cellStyle);
        $cell->addText($v[$i]);
    }
}
$section->addText("");
...
foreach($wordData['total_price_data']['key'] as $k=>$v){
    $cell = $table->addCell(3000,$cellStyle);
    $cell->addText($v,array('bold'=>true));
}
...
$table->addRow();
$cell = $table->addCell(5000,$cellStyle);
$cell->addText("费用仅包含",array('bold'=>true));
$cell = $table->addCell(5000,$cellStyle);
$cell->addText('费用不包含',array('bold'=>true));
...
⑵建议甲方出行后不在海上或礁石上、交通公路上、交通工具上、禁止拍照的地方或高山危险地方拍照，一切行为要充分参考当地、当时相关公告及建议，出现意外伤害、财产损失，或购买到假冒商品，乙方均不担责。";
	    ...
	    foreach($str51 as $k=>$v){
		    $section->addText($v,array('bold'=>true),array('spacing'=>115));
	    }
...
浮潜";
	    ...
	    foreach($str53 as $k=>$v){
		    $section->addText($v,array('bold'=>true),array('spacing'=>115));
	    }

$str54 = "1、醉酒者、患有耳、鼻疾病、癫痫症、精神病、结核病、糖尿病、肾脏病、心脏病、气喘、高（低）血压等疾病的游客不能从事潜水活动；低于10岁的儿童不能从事潜水活动。以上疾病类型只是简要示例，如游客尚有其他疾病可能不适合参加旅游活动的，请主动向旅行社告知或咨询。
...
$str55 = "快艇";
	    ...
	    foreach($str55 as $k=>$v){
		    $section->addText($v,array('bold'=>true),array('spacing'=>115));
	    }

$str56 = "1、严禁携带易燃、易爆、腐蚀性等危及人身安全的物品上快艇。
...
$str57 = "潜水";
	    ...
	    foreach($str57 as $k=>$v){
		    $section->addText($v,array('bold'=>true),array('spacing'=>115));
	    }

$str58 = "1、醉酒者、患有耳、鼻疾病、癫痫症、精神病、结核病、糖尿病、肾脏病、心脏病、气喘、高（低）血压等疾病的游客不能从事潜水活动；低于10岁的儿童不能从事潜水活动。以上疾病类型只是简要示例，如游客尚有其他疾病可能不适合参加旅游活动的，请主动向旅行社告知或咨询。
2、游客境外出游的，在自由活动期间，切勿参加非法或未经中国政府核实的当地旅游团体提供的自费项目、行程，以免发生人身伤亡、财产损失、食物中毒等意外事件。
...
$section->addText('“卫蓝”文明出行倡议',array('size'=>'22','bold'=>true,'name'=>'微软雅黑'),array('align'=>'center','spacing'=>115,'name'=>'微软雅黑'));
$section->addText('');
...
$section->addText('');
$section->addText('遇·岛热心于社会公益，营业额的百分之一，我们会捐给广州卫蓝自然环境保护协会，用于环保公益事业。',array('bold'=>true),array('align'=>'center','spacing'=>115));


//设置页头
$header = $section->createHeader();
$header->addImage(APP_PATH.'components/PHPWord_Sam/'.'1.png',array('_width'=>110,'_height'=>60,'align'=>'left','zIndex'=>10,'yIndex'=>10,'_marginLeft'=>0.1,'_marginTop'=>0.1));
$header->addText('');
